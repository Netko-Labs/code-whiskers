...
const GRID = '1fr 220px 100px 100px 80px 110px'
const COLUMNS = [
  { label: 'Directory' },
  { label: 'Repository' },
  { label: 'Findings' },
...
export function useCodebaseMapSection(tab: number): SectionDefinition {
  ...
  return useMemo(() => {
    ...
    const findings = spots.reduce((sum, s) => sum + s.findings, 0)
    ...
    const table: SectionTable = {
      ...
      footer: 'Latest review of each pull request, last 90 days · directories three levels deep',
    }
    ...
    return {
      ...
      stats: [
        ...
        { label: 'Findings', value: String(findings), note: 'latest review per PR' },
