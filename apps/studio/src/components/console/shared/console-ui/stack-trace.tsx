...
export function StackTrace({ title, frames, hiddenNote }: StackTraceProps) {
  return (
    <div className="flex-1 overflow-auto bg-zinc-950 px-4 py-3.5 font-mono text-xs leading-5 text-zinc-400">
      ...
      {frames.map((frame, index) => (
        <div key={`${frame.no}-${index}`} className="flex gap-2.5">
          ...
          <span className="w-2.5 text-sev-critical">{frame.current ? '→' : ''}</span>
