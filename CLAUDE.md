...
- Two apps:
  - `apps/studio` — TanStack Start (React 19, Tailwind, Base UI, Tabler Icons) frontend + an Elysia backend for everything a human creates or decides: auth (better-auth: GitHub OAuth + jwt/jwks), GitHub sync, triage decisions and comments, review rules, API keys, webhook integrations, alert rules, saved queries. Owns the **public hostname** and forwards the whiskers surfaces to the worker via `forwardToWhiskers` (`packages/studio/api/src/shared`), byte-for-byte so HMAC/DSN checks still happen in whiskers: `/webhooks/*`, `/api/:projectId/envelope|store` (Sentry), `/otlp/*` (OTLP/HTTP JSON, project key) and `/v1/*` (the read API — a browser session or a read-only `cw_` API key; POST needs a session). `WHISKERS_URL` points at the worker (internal DNS in Coolify).
...
- **Two databases, one rule**: studio owns what a human creates or configures, whiskers owns what machines produce at volume. Studio is the source of truth; whiskers is a worker. They talk over HTTP (`/v1/*` out, `/api/internal/*` back) and never open each other's connection — no cross-database foreign keys. Full table-by-table breakdown in @docs/architecture.md.
...
- `apps/studio` backend: better-auth at `/api/auth` (GitHub OAuth + `jwt`/`jwks`), plus the human-owned API — `/api/triage`, `/api/rules`, `/api/keys`, `/api/integrations`, `/api/alerts`, `/api/saved-queries`, `/api/members`, `/api/instance`. Machine-produced data (reviews, findings, issues, events, logs, spans) lives in whiskers. Whiskers reads human decisions from studio's `/api/internal/*` (suppressions, rules, alert rules, repository watch) with `INTERNAL_TOKEN`, and asks studio to deliver alerts so webhook secrets never leave studio. Put `drizzle-zod` entities in the relevant `domain` package (`createInsertSchema()`/`createUpdateSchema()`/`createSelectSchema()`).
