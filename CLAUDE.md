...
- `apps/studio` backend is **auth only**: better-auth is mounted at `/api/auth` (GitHub OAuth + `jwt`/`jwks`). All transactional data + logic lives in whiskers. Put `drizzle-zod` entities in the relevant `domain` package (`createInsertSchema()`/`createUpdateSchema()`/`createSelectSchema()`).
