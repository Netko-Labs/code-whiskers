...
export const getTraces = async (service?: string): Promise<TraceSummary[]> => {
...
export const getServices = async (): Promise<ServiceSummary[]> => {
  const rows = (await db.execute(sql`
    with logs as (
      select service, count(*) as logs,
             count(*) filter (where level in ('ERROR', 'FATAL')) as log_errors,
             max(timestamp) as last_log
      from log_line where timestamp > now() - interval '1 day' group by service
    ),
    spans as (
      ...
      from span where start_time > now() - interval '1 day' group by service
    )
